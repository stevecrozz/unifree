...
use crate::types::{Band, RadioTableEntry};
...
impl ToUnifiConfig for RadioConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        let mut kvs = vec![
            ...
            (format!("{}.bcmc_l2_filter.status", p), self.bcmc_l2_filter_status.to_string()),
            (format!("{}.bgscan.status", p), self.bgscan_status.to_string()),
            (format!("{}.hard_noisefloor.status", p), self.hard_noisefloor_status.to_string()),
        ];
...
impl IniSection for RadioSection {
    fn generate(&self, config: &ProvisionConfig, device_state: &DeviceState, mac: &str) -> HashMap<String, Vec<String>> {
        ...
        add_line(&mut sections, h, format!("radio.countrycode={}", country_code));
        ...
        for radio in &radios {
             ...
             let radio_cfg = RadioConfig {
                 ...
                 freq: if radio.freq > 0 { Some(radio.freq) } else { None },
...
pub fn get_phys_radios(config: &ProvisionConfig, device_state: &DeviceState, mac: &str) -> Vec<PhysRadio> {
    ...
    if radios.is_empty() {
         radios = vec![
            PhysRadio { idx: 1, phyname: "wifi0".to_string(), mode: "11nght20".to_string(), band: Band::Band2g, antenna_gain: 4, txpower_mode: "high".to_string(), freq: 0 },
            PhysRadio { idx: 2, phyname: "wifi1".to_string(), mode: "11naht40".to_string(), band: Band::Band5g, antenna_gain: 6, txpower_mode: "high".to_string(), freq: 0 },
            PhysRadio { idx: 3, phyname: "wifi2".to_string(), mode: "11naht160".to_string(), band: Band::Band6g, antenna_gain: 6, txpower_mode: "auto".to_string(), freq: 6135 },
        ];
    }
    radios
}
...
fn entry_to_phys_radio(i: usize, entry: &RadioTableEntry) -> PhysRadio {
    ...
    let antenna_gain = entry.antenna_gain.as_ref().and_then(|v| v.as_u64()).map(|v| v as u8).unwrap_or(0);
