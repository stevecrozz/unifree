...
impl ToUnifiConfig for NetconfConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        let mut kvs = vec![
            ...
            (format!("{}.autoip.status", p), self.autoip_status.to_string()),
        ];
...
impl ToUnifiConfig for BridgeConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        for port in &self.ports {
            kvs.push((format!("{}.port.{}.devname", p, port.idx), port.devname.clone()));
        }
        kvs
    }
}
...
impl ToUnifiConfig for EbtablesConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        vec![
            (format!("{}.cmd", p), self.cmd.clone()),
        ]
    }
}

impl IniSection for NetworkSection {
    fn generate(&self, config: &ProvisionConfig, device_state: &DeviceState, mac: &str) -> HashMap<String, Vec<String>> {
        ...
        let mut netconf_raw = vec![
            ("br0".to_string(), "0.0.0.0".to_string(), "enabled".to_string(), "enabled".to_string()),
            ("eth0".to_string(), "0.0.0.0".to_string(), "enabled".to_string(), "enabled".to_string()),
        ];
        ...
        for radio in &radios {
             ...
             for (_net_name, net_config) in radio_networks {
                 ...
                 netconf_raw.push((devname.clone(), "0.0.0.0".to_string(), "enabled".to_string(), "disabled".to_string()));
                 
                 ebtables_rules.push(format!("-t nat -A PREROUTING --in-interface {} -d BGA -j DROP", devname));
                 ebtables_rules.push(format!("-t nat -A POSTROUTING --out-interface {} -d BGA -j DROP", devname));
                 
                 if let Some(vlan_id) = net_config.vlan {
                     vlans_seen.insert(vlan_id);
                     vlan_defs.push((vlan_id, devname.clone()));
                     ebtables_rules.push(format!("-t broute -A BROUTING -i {} -p 802_1Q -j DROP", devname));
                 } else {
        ...
        if !vlans_seen.is_empty() {
            ...
            for vid in &sorted_vids {
                ...
                netconf_raw.push((format!("eth0.{}", vid), "0.0.0.0".to_string(), "enabled".to_string(), "enabled".to_string()));
            }
        } else {
        ...
        // --- Bridge ---
        add_line(&mut sections, " bridge", "bridge.status=enabled".to_string());
        
        // br0
        let mut br0_ports_cfg = Vec::new();
        for (i, port) in br0_ports.iter().enumerate() {
            br0_ports_cfg.push(BridgePortConfig { idx: (i + 1) as u8, devname: port.clone() });
        }
        ...
        if !vlans_seen.is_empty() {
             ...
             for vid in sorted_vids {
                ...
                for (v, dev) in &vlan_defs {
                    if *v == vid && dev != "eth0" {
                        br_ports.push(BridgePortConfig { idx: port_idx, devname: dev.clone() });
                        port_idx += 1;
                    }
                }
                // Add eth0.VID
                br_ports.push(BridgePortConfig { idx: port_idx, devname: format!("eth0.{}", vid) });
                ...
                netconf_raw.push((br_name, "0.0.0.0".to_string(), "enabled".to_string(), "enabled".to_string()));
                ebtables_rules.push(format!("-t broute -A BROUTING --vlan-id {} -p 802_1Q -j DROP", vid));
        ...
        // --- Netconf ---
        add_line(&mut sections, " netconf", "netconf.status=enabled".to_string());
        for (i, (dev, ip, status, up)) in netconf_raw.iter().enumerate() {
            let idx = (i + 1) as u8;
            let is_wireless = dev.contains("wifi") || dev.contains("ath") || dev.contains("vwire") || dev.contains("scan");
            let final_up = if is_wireless { UnifiBool::Disabled } else { if up == "enabled" { UnifiBool::Enabled } else { UnifiBool::Disabled } };
            let status_bool = if status == "enabled" { UnifiBool::Enabled } else { UnifiBool::Disabled };
        ...
        // --- Ebtables ---
        add_line(&mut sections, " ebtables", "ebtables.status=enabled".to_string());
        add_line(&mut sections, " ebtables", "ebtables.add_vlan.status=disabled".to_string());
