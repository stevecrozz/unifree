...
impl ToUnifiConfig for WirelessConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        let mut kvs = vec![
            ...
            (format!("{}.mac_acl.status", p), self.mac_acl_status.to_string()),
            ...
            (format!("{}.element_adopt", p), self.element_adopt.to_string()),
            (format!("{}.is_guest", p), self.is_guest.to_string()),
            (format!("{}.mcast.enhance", p), self.mcast_enhance.to_string()),
            ...
            (format!("{}.schedule_enabled", p), self.schedule_enabled.to_string()),
...
impl ToUnifiConfig for AaaConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        ...
        let mut kvs = vec![
            ...
            (format!("{}.bss_transition", p), self.bss_transition.to_string()),
            (format!("{}.country_beacon", p), self.country_beacon.to_string()),
            (format!("{}.eapol_version", p), self.eapol_version.to_string()),
            (format!("{}.ft.status", p), self.ft_status.to_string()),
            (format!("{}.hide_ssid", p), self.hide_ssid.to_string()),
            (format!("{}.is_guest", p), self.is_guest.to_string()),
            (format!("{}.p2p", p), self.p2p.to_string()),
            (format!("{}.p2p_cross_connect", p), self.p2p_cross_connect.to_string()),
            (format!("{}.proxy_arp", p), self.proxy_arp.to_string()),
            (format!("{}.radius.macacl.status", p), self.radius_macacl_status.to_string()),
            (format!("{}.tdls_prohibit", p), self.tdls_prohibit.to_string()),
            (format!("{}.wpa.group_rekey", p), self.wpa_group_rekey.to_string()),
            (format!("{}.id", p), self.unique_id.clone()),
        ...
        for radio in &radios {
            ...
            for (_net_name, net_config) in radio_networks {
                 ...
                 let wireless_cfg = WirelessConfig {
                     ...
                     l2_isolation: if net_config.client_device_isolation { UnifiBool::Enabled } else { UnifiBool::Disabled },
                     ...
                     beacon_rate: if radio.band == Band::Band2g { Some(1000) } else { None },
                     dtim_period: if radio.band == Band::Band2g { 1 } else { 3 },
                     element_adopt: UnifiBool::Disabled,
                     is_guest: false,
                     mcast_enhance: 0,
                     mcastrate: "auto".to_string(),
                     mgmt_rate: if radio.band == Band::Band2g { Some(1000) } else { None },
                     minrate_cck_rates_status: if radio.band == Band::Band2g { Some(true) } else { None },
                     minrate_data: if radio.band == Band::Band2g { Some(1000) } else { None },
                     no2ghz_oui: if radio.band == Band::Band2g && !net_config.iot { UnifiBool::Enabled } else { UnifiBool::Disabled },
                     puren: 0,
                     ...
                     iot: if net_config.iot { Some(UnifiBool::Enabled) } else { None },
                     qbssload: if net_config.iot { Some(UnifiBool::Disabled) } else { None },
                 ...
                 let mut aaa_cfg = AaaConfig {
                     ...
                     status: UnifiBool::Enabled,
                     ssid: net_config.ssid.clone(),
                     br_devname: if net_config.vlan.is_some() { format!("br0.{}", net_config.vlan.unwrap()) } else { "br0".to_string() },
                     driver: "madwifi".to_string(),
                     verbose: 2,
                     k11_status: UnifiBool::Disabled,
                     bss_transition: if net_config.iot { UnifiBool::Disabled } else { UnifiBool::Enabled },
                 ...
                 match net_config.security {
                    ...
                    SecurityMode::Wpa2Wpa3 => {
                        ...
                        aaa_cfg.wpa3_transition = if radio.band == Band::Band6g { Some(UnifiBool::Disabled) } else { Some(UnifiBool::Enabled) };
