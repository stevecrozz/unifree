...
impl ProvisionConfig {
    ...
    pub fn generate_mgmt_cfg_with_auth(
        ...
        // cfgversion from device.json
        if let Some(d) = device {
            lines.push(format!("cfgversion={}", d.cfgversion));
        } else {
        ...
        if let Some(d) = device {
            if let Some(inform_url) = &d.inform_url {
                lines.push(format!("mgmt_url={}", inform_url.replace(":8081/inform", ":8443/manage/site/default")));
            }
            if let Some(inform_ip) = &d.inform_ip {
                // Check for stun_url in settings, fallback to inform_ip
                let stun_url = self.management.stun_url.clone() 
