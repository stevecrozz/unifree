...
use std::fs;
use std::path::Path;
use unifree_config::config::models::{DeviceInfo, ProvisionConfig};
...
fn test_generate_system_ini_matches_reference() {
    ...
    let device_info: DeviceInfo =
        serde_json::from_str(&device_content).expect("Failed to parse device.json fixture");
    ...
    let gen_lines: HashSet<String> = generated
        .lines()
        .map(|l| normalize(l))
        .filter(|l| !l.is_empty())
        .collect();

    let ref_lines: HashSet<String> = reference_content
        .lines()
        .map(|l| normalize(l))
        .filter(|l| !l.is_empty())
        .collect();
    ...
    if !missing_in_gen.is_empty() || !extra_in_gen.is_empty() {
        ...
        if !missing_in_gen.is_empty() {
            println!(
                "\nMissing in generated config ({} lines):",
                missing_in_gen.len()
            );
        ...
        if !extra_in_gen.is_empty() {
            println!(
                "\nExtra in generated config ({} lines):",
                extra_in_gen.len()
            );
