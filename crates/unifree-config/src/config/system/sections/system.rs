...
impl ToUnifiConfig for UnifiConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        vec![
            ...
            (
                "unifi.anonymous_controller_id".to_string(),
                self.anonymous_controller_id.clone(),
            ),
            (
                "unifi.anonymous_site_id".to_string(),
                self.anonymous_site_id.clone(),
            ),
            ...
            (
                "unifi.feature.always_send_crash_logs".to_string(),
                self.feature_always_send_crash_logs.to_string(),
            ),
...
impl IniSection for SystemSection {
    fn generate(
        ...
        let unifi_key = config
            .devices_info
            .get(&mac.replace(":", "").to_lowercase())
...
impl ToUnifiConfig for SystemConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        vec![
            (
                "system.analytics.status".to_string(),
                self.analytics_status.to_string(),
            ),
