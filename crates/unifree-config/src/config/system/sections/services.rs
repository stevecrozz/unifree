...
impl ToUnifiConfig for SyslogRemoteConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        let mut kvs = vec![
            ...
            (
                "syslog.remote.encrypt".to_string(),
...
impl ToUnifiConfig for ResolvHostConfig {
    fn to_config(&self) -> Vec<(String, String)> {
        vec![(format!("resolv.host.{}.name", self.idx), self.name.clone())]
    }
}
...
impl IniSection for ServicesSection {
    fn generate(
        ...
        add_line(
            &mut sections,
            " connectivity",
            "connectivity.status=disabled".to_string(),
        );

        // --- dhcpc ---
        add_line(&mut sections, " dhcpc", "dhcpc.status=enabled".to_string());
        let dhcpc = DhcpcConfig {
            idx: 1,
            devname: "br0".to_string(),
            status: UnifiBool::Enabled,
        };
        for (k, v) in dhcpc.to_config() {
            add_line(&mut sections, " dhcpc", format!("{}={}", k, v));
        }
        ...
        add_line(
            &mut sections,
            " dnsmasq",
            "dnsmasq.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " iptables",
            "iptables.status=disabled".to_string(),
        );
        add_line(
            &mut sections,
            " iptables",
            "ip6tables.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " mac acl",
            "macacl.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " ntpclient",
            "ntpclient.status=enabled".to_string(),
        );
        if !config.ntp_servers.is_empty() {
            for (i, server) in config.ntp_servers.iter().enumerate() {
                let ntp = NtpClientConfig {
                    idx: (i + 1) as u8,
                    server: server.clone(),
                    status: UnifiBool::Enabled,
                };
                for (k, v) in ntp.to_config() {
                    add_line(&mut sections, " ntpclient", format!("{}={}", k, v));
                }
            }
        } else {
            for i in 1..=4 {
                let ntp = NtpClientConfig {
                    idx: i as u8,
                    server: format!("{}.ubnt.pool.ntp.org", i - 1),
                    status: UnifiBool::Enabled,
                };
                for (k, v) in ntp.to_config() {
                    add_line(&mut sections, " ntpclient", format!("{}={}", k, v));
        ...
        add_line(
            &mut sections,
            " redirector",
            "redirector.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " resolv",
            "resolv.status=enabled".to_string(),
        );
        let normalized_mac = mac.replace(":", "").to_lowercase();
        if let Some(device) = config.devices.get(&normalized_mac) {
            if let Some(name) = &device.name {
                let rhost = ResolvHostConfig {
                    idx: 1,
                    name: name.clone(),
                };
                for (k, v) in rhost.to_config() {
                    add_line(&mut sections, " resolv", format!("{}={}", k, v));
                }
            }
        }
        add_line(
            &mut sections,
            " resolv",
            "resolv.nameserver.1.status=disabled".to_string(),
        );
        add_line(
            &mut sections,
            " resolv",
            "resolv.nameserver.2.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " sshd",
            "sshd.auth.passwd=enabled".to_string(),
        );
        ...
        for (i, key) in all_keys.iter().enumerate() {
            ...
            for (k, v) in kcfg.to_config() {
                add_line(&mut sections, " sshd", format!("{}={}", k, v));
            }
        }
        let sshd_if = SshdInterfaceConfig {
            idx: 1,
            status: UnifiBool::Enabled,
            ifname: "br0".to_string(),
        };
        for (k, v) in sshd_if.to_config() {
            add_line(&mut sections, " sshd", format!("{}={}", k, v));
        }
        ...
        add_line(
            &mut sections,
            " stamgr",
            "stamgr.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " switch",
            "switch.status=disabled".to_string(),
        );
        ...
        add_line(
            &mut sections,
            " syslog",
            "syslog.status=enabled".to_string(),
        );
        ...
        if let Some(device) = config.devices_info.get(&normalized_mac) {
            if let Some(ip) = &device.inform_ip {
                ...
                for (k, v) in syslog.to_config() {
                    add_line(&mut sections, " syslog", format!("{}={}", k, v));
                }
                ...
                for (k, v) in netconsole.to_config() {
                    add_line(&mut sections, " syslog", format!("{}={}", k, v));
                }
            }
        }
