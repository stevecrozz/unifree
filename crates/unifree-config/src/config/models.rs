...
use unifree_types::{Band, RadioTableEntry, SecurityMode};
...
pub struct DeviceConfig {
    ...
    /// Per-network configurations for this device (overrides global defaults)
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub networks: HashMap<String, NetworkConfig>,

    /// List of default network names to disable for this device
...
impl ProvisionConfig {
    ...
    pub fn get_networks_for_device(&self, mac_str: &str) -> HashMap<String, NetworkConfig> {
        let normalized_mac = mac_str.replace(":", "").to_lowercase();
        let mut effective_networks = self.networks.clone(); // Start with global defaults

        if let Some(device_config) = self.devices.get(&normalized_mac) {
            // Apply device-specific network overrides
            for (name, config) in &device_config.networks {
                effective_networks.insert(name.clone(), config.clone());
            }

            // Remove disabled networks
            for disabled_name in &device_config.disabled_networks {
                effective_networks.remove(disabled_name);
            }
        }
